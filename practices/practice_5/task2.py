...
# desired state
ad = np.array([pi/3, pi/6])
dad = np.array([0,0])
# p_gains = np.array([300,280])
# d_gains = np.array([30,25])

p_gains = np.array([10,3])
d_gains = np.array([0.03,0.05])

# p_gains = np.array([0.0,0.0])
# d_gains = np.array([0.0,0.0])
...
try:
    ...
    while True:
        ...
        if (time - last_execution) >= sampling_time:
            ...
            control = -(p_gains.dot(a_error) + d_gains.dot(da_error))
            # control = zeros(2)
    ...
    ax[0].plot(ts, np.full(N,ad[0]), label=f"$\\alpha_{{1,desired}}$ = {ad[0]:.3f}", linestyle="-.", linewidth=3)
    ax[0].plot(ts, np.full(N,ad[1]), label=f"$\\alpha_{{2,desired}}$ = {ad[1]:.3f}", linestyle="-.", linewidth=3)
    ax[0].legend()
    ...
    ax[1].plot(ts, np.full(N,dad[0]), label=f"$\\dot{{\\alpha}}_{{1,desired}}$ = {dad[0]:.3f}", linestyle="-.", linewidth=3)
    ax[1].plot(ts, np.full(N,dad[1]), label=f"$\\dot{{\\alpha}}_{{2,desired}}$ = {dad[1]:.3f}", linestyle="-.", linewidth=3)
    ax[1].legend()


    fig.suptitle(f"PD control with P gains = {p_gains[0]:.3f}, {p_gains[1]:.3f}; D gains = {d_gains[0]:.3f}, {d_gains[1]:.3f}", fontsize=16)
